...
async function registerWithRetry(
    ...
    for (let attempt = 1; attempt <= retries; attempt++) {
        try {
            ...
            console.log("[mfe-notifications] Registered with registry");
            return;
        } catch (err) {
            console.warn(
                `[mfe-notifications] Registry unavailable (attempt ${attempt}/${retries}):`,
                err
            );
            if (attempt === retries) {
                console.error("[mfe-notifications] Giving up on registry registration");
                return;
            }
